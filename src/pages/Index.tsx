...
import { Shield, Lock, Video, Smartphone, Heart, Users, Sprout, ShieldCheck } from "lucide-react";
...
const STRIP = [
  {
    ...
    icon: Shield,
    label: "Scams & Safety",
    count: "1,240 questions answered",
  },
  {
    key: "Passwords & Accounts",
    icon: Lock,
    label: "Passwords & Accounts",
    count: "980 questions answered",
  },
  {
    key: "Video Calls",
    icon: Video,
    label: "Video Calls",
    count: "743 questions answered",
  },
  {
    key: "Apps & Phone Settings",
    icon: Smartphone,
    label: "Apps & Phone Settings",
    count: "1,105 questions answered",
  },
  {
    key: "Health & News",
    icon: Heart,
    label: "Health & News",
    count: "621 questions answered",
  },
] as const;
      ...
      <section className="border-b border-[#E8E7E0] px-4 py-10 md:px-6 md:py-16">
        <div className="mx-auto max-w-[680px] text-center">
          <h1 className="mb-4 text-3xl font-medium leading-tight text-[#2C2C2A] md:text-4xl md:leading-tight">
            Get help with your phone, tablet, or computer — from real people who understand
          </h1>
          ...
          <div className="mb-6 flex min-h-[3rem] flex-col items-stretch justify-center gap-3 sm:flex-row sm:justify-center">
            ...
            <Button
              asChild
              variant="outline"
              className="h-auto rounded-lg border border-[#1D9E75] bg-transparent px-6 py-3 text-base font-medium text-[#1D9E75] transition-colors duration-200 hover:bg-[#1D9E75]/5 min-h-[2.5rem] shadow-none"
            >
              <Link to="/browse">Browse answers</Link>
            </Button>
          </div>
      ...
      <section className="border-b border-[#E8E7E0] px-4 py-10 md:px-6 md:py-16">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {STRIP.map(({ key, icon: Icon, label, count }) => (
              <Link
                key={key}
                to={`/browse?category=${encodeURIComponent(key)}`}
                className="group flex min-h-[8rem] flex-col items-center justify-center gap-3 rounded-xl border border-[#E8E7E0] bg-white p-5 text-center transition-colors duration-200 hover:border-[#1D9E75]/40"
              >
                <span className="inline-flex flex-col items-center gap-2">
                  <span className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#F0FAF5] text-[#1D9E75] transition-colors duration-200 group-hover:bg-[#E3F5EC]">
            ...
            {WHY.map((item) => (
              ...
              >
                <div className="mb-3 inline-flex items-center gap-2 text-[#1D9E75]">
                  <item.icon className="h-7 w-7 shrink-0" strokeWidth={1.75} aria-hidden />
